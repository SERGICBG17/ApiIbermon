...
IBERMON = [
    {
        ...
        "movimientos_posibles": [1, 2, 6, 7], "evoluciona_a": 2, "nivel_evolucion": 16,
    },
    {
        ...
        "movimientos_posibles": [1, 6, 7, 8], "evoluciona_a": 3, "nivel_evolucion": 36,
    },
    {
        ...
        "movimientos_posibles": [1, 6, 8, 9], "evoluciona_a": None, "nivel_evolucion": None,
    },
    {
        "numero": 4, "nombre": "Aquillo", "tipo1": "Agua", "tipo2": None,
        "descripcion": "Pequeno ibermon de agua. Starter inicial. Nada a gran velocidad.",
        "stats_base": {"hp": 44, "ataque": 48, "defensa": 65, "ataque_especial": 50, "defensa_especial": 64, "velocidad": 43},
        "movimientos_posibles": [1, 3, 10, 11], "evoluciona_a": 5, "nivel_evolucion": 16,
    },
    {
        ...
        "movimientos_posibles": [1, 10, 11, 13], "evoluciona_a": 6, "nivel_evolucion": 36,
    },
    {
        ...
        "movimientos_posibles": [1, 10, 12, 13], "evoluciona_a": None, "nivel_evolucion": None,
    },
    {
        "numero": 7, "nombre": "Verdino", "tipo1": "Planta", "tipo2": None,
        "descripcion": "Pequeno ibermon de planta. Starter inicial. Lleva un bulbo en su espalda.",
        "stats_base": {"hp": 45, "ataque": 49, "defensa": 49, "ataque_especial": 65, "defensa_especial": 65, "velocidad": 45},
        "movimientos_posibles": [1, 4, 14, 17], "evoluciona_a": 8, "nivel_evolucion": 16,
    },
    {
        ...
        "movimientos_posibles": [1, 14, 15, 17], "evoluciona_a": 9, "nivel_evolucion": 36,
    },
    {
        ...
        "movimientos_posibles": [1, 15, 16, 17], "evoluciona_a": None, "nivel_evolucion": None,
    },
    {
        "numero": 10, "nombre": "Chispon", "tipo1": "Electrico", "tipo2": None,
        "descripcion": "Ibermon electrico comun en los campos. Sus mejillas almacenan electricidad.",
        "stats_base": {"hp": 35, "ataque": 55, "defensa": 40, "ataque_especial": 50, "defensa_especial": 50, "velocidad": 90},
        "movimientos_posibles": [1, 5, 18, 20], "evoluciona_a": 11, "nivel_evolucion": 22,
    },
    {
        ...
        "movimientos_posibles": [5, 18, 19, 20], "evoluciona_a": None, "nivel_evolucion": None,
    },
    {
        "numero": 12, "nombre": "Gusanin", "tipo1": "Bicho", "tipo2": None,
        "descripcion": "Pequeno ibermon insecto. Muy debil pero facil de encontrar al inicio.",
        "stats_base": {"hp": 45, "ataque": 30, "defensa": 35, "ataque_especial": 20, "defensa_especial": 20, "velocidad": 45},
        "movimientos_posibles": [1, 2, 3], "evoluciona_a": 13, "nivel_evolucion": 7,
    },
    {
        "numero": 13, "nombre": "Capuller", "tipo1": "Bicho", "tipo2": None,
        "descripcion": "Crisalida de Gusanin. Endurece su caparazon para protegerse.",
        "stats_base": {"hp": 50, "ataque": 35, "defensa": 55, "ataque_especial": 25, "defensa_especial": 25, "velocidad": 30},
        "movimientos_posibles": [1, 4], "evoluciona_a": 14, "nivel_evolucion": 10,
    },
    {
        ...
        "movimientos_posibles": [1, 5, 15, 17], "evoluciona_a": None, "nivel_evolucion": None,
    },
    {
        ...
        "movimientos_posibles": [1, 3, 4], "evoluciona_a": 16, "nivel_evolucion": 25,
    },
    {
        ...
        "movimientos_posibles": [1, 3, 4, 5], "evoluciona_a": None, "nivel_evolucion": None,
    },
]
...
async def seed_movimientos():
    ...
    if count > 0:
        print(f"  Ya hay {count} movimientos en la BD, omitiendo...")
        return
    docs = [MovimientoCatalogo(**m) for m in MOVIMIENTOS]
    await MovimientoCatalogo.insert_many(docs)
    print(f"  {len(docs)} movimientos insertados")


async def seed_ibermon():
    count = await IbermonCatalogo.count()
    if count > 0:
        print(f"  Ya hay {count} ibermon en la BD, omitiendo...")
        return
    docs = [
        IbermonCatalogo(**{**ib, "stats_base": StatsBase(**ib["stats_base"])})
        for ib in IBERMON
    ]
    await IbermonCatalogo.insert_many(docs)
    print(f"  {len(docs)} ibermon insertados")


async def seed_items():
    count = await ItemCatalogo.count()
    if count > 0:
        print(f"  Ya hay {count} items en la BD, omitiendo...")
        return
    docs = [
        ItemCatalogo(**{**it, "efecto": EfectoItem(**it["efecto"])})
        for it in ITEMS
    ]
    await ItemCatalogo.insert_many(docs)
    print(f"  {len(docs)} items insertados")


async def seed_logros():
    count = await LogroCatalogo.count()
    if count > 0:
        print(f"  Ya hay {count} logros en la BD, omitiendo...")
        return
    docs = [LogroCatalogo(**l) for l in LOGROS]
    await LogroCatalogo.insert_many(docs)
    print(f"  {len(docs)} logros insertados")
